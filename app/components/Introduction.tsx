...
const FeatureItem = ({ icon: Icon, title, description, delay }: { icon: any, title: string, description: string, delay: number }) => (
  <motion.div 
    ...
    className="flex flex-col items-center text-center p-8 bg-white/5 border border-white/10 rounded-2xl hover:border-gold-500/50 hover:bg-white/[0.07] transition-colors group cursor-default"
  >
    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-gold-500/20 to-transparent flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-500 ease-out">
      <Icon className="w-10 h-10 text-gold-500 drop-shadow-[0_0_10px_rgba(212,175,55,0.5)]" />
    </div>
    <h3 className="text-2xl font-serif font-bold text-white mb-4">{title}</h3>
    <p className="text-gray-400 leading-relaxed text-lg font-light">{description}</p>
  </motion.div>
);
    ...
    <Section className="bg-luxury-charcoal py-32">
      <Container>
        <div className="text-center max-w-4xl mx-auto mb-24">
          ...
          >
            More Than a Tool. <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold-300 to-gold-600">It's Your Digital Headquarters.</span>
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0 }}
